...
2. Per-folder and per-device variables and feedbacks, with dropdowns populated from the configuration
3. Per-folder and per-device actions: pause, resume, rescan, override, revert
4. Event stream via `/rest/events` long polling, replacing most of the polling
5. Presets per folder and per device, then submission to the Bitfocus module list
